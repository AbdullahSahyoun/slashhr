...
const InitialBadge = ({ name }) => {
  ...
  return (
    <div className="h-[64px] w-[64px] rounded-2xl grid place-items-center font-semibold text-white"
         style={{ background: "#2b6171" }}>
      {letter}
    </div>
  );
};

export default function Update({ open, onClose, team, onSaved }) {
  ...
  const save = async () => {
    if (!team?.id || !canSave) return;
    setSaving(true); setErr("");
    try {
      const body = { name: name.trim(), description: description.trim() };
      const updated = await patchJSON(`${API_ORIGIN}/org/team/${team.id}`, body);
      onSaved?.(updated);
      onClose?.();
    } catch (e) {
      try {
      ...
      <div className="absolute inset-0 flex items-start justify-center p-4 sm:p-6">
        <div className="w-full max-w-[560px] rounded-2xl bg-white shadow-xl overflow-hidden">
          <div className="px-5 pt-5 pb-3 border-b border-gray-100 flex items-start justify-between">
            <div>
              <h3 className="text-[18px] font-semibold text-[#2b6171]">Edit Team Information</h3>
              <p className="text-xs text-gray-500 mt-1">Select a team name and include a description if needed.</p>
            </div>
            <button
          ...
          <div className="px-5 py-4 space-y-4">
            <div className="flex items-center gap-4">
              <InitialBadge name={name || team?.name} />
              {/* (optional) add avatar edit icon if you implement it later */}
            </div>

            {err && <div className="text-sm text-red-600">{err}</div>}

            <label className="block">
              <span className="block text-sm text-gray-700 mb-1">Team name</span>
              <input
