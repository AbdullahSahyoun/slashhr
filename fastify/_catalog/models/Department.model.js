// src/_organization/models/departments.model.js
...
export async function getAll(fastify, tenantId, { q, active = true, limit = 100, offset = 0 } = {}) {
  ...
  if (q && String(q).trim() !== '') {
    ...
    where.push(`department_name ILIKE $${params.length}`);
  }
  ...
  const sql = `
    SELECT id, department_name AS label, is_active, "TenantID"
...
/** Create department. */
export async function create(fastify, tenantId, department_name) {
  const sql = `
    INSERT INTO organization.departments ("TenantID", department_name)
    VALUES ($1, $2)
    RETURNING id, department_name AS label, is_active, "TenantID";
  `;
  const { rows } = await fastify.pg.query(sql, [tenantId, department_name]);
  return rows[0];
}
...
export async function update(fastify, tenantId, id, patch = {}) {
  ...
  if (patch.department_name !== undefined) {
    params.push(patch.department_name);
    sets.push(`department_name = $${params.length}`);
  }
